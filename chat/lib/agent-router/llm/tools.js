 ...
 */
export function commandIdToToolName(commandId) {
    const name = commandId.split('.').join(DOT_ESCAPE);
    if (!TOOL_NAME_PATTERN.test(name)) {
        throw new Error(`Não foi possível derivar um nome de tool válido de "${commandId}" ` +
 ...
 */
export function paramToJsonSchema(param) {
...
export function commandToToolSchema(command) {
    const properties = {};
    const required = [];
 ...
 */
export function toToolSchemas(candidates) {
...
export function clarifyTool() {
    return {
        ...
        description: 'Use quando NENHUM dos comandos disponíveis claramente atende ao pedido, ou quando ' +
        ...
        input_schema: {
            ...
            properties: {
                pergunta: {
                    type: 'string',
                    description: 'Pergunta objetiva, em pt-BR, que ajude o usuário a esclarecer o que ele quer.',
                },
                motivo: {
                    type: 'string',
