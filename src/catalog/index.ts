import type { Catalog, CommandDef, AgentId } from '../types.js';
...
const byId = new Map<string, CommandDef>(catalog.commands.map((c) => [c.id, c]));

export function getCommand(id: string): CommandDef | undefined {
  return byId.get(id);
}

export function commandsForAgent(agent: AgentId): CommandDef[] {
 ...
 */
export function validateCatalog(c: Catalog = catalog): string[] {
  const problems: string[] = [];
  const seen = new Set<string>();
  const agentIds = new Set(c.agents.map((a) => a.id));

  for (const cmd of c.commands) {
    if (seen.has(cmd.id)) problems.push(`id duplicado: ${cmd.id}`);
    seen.add(cmd.id);

    if (!agentIds.has(cmd.agent)) problems.push(`${cmd.id}: agente desconhecido "${cmd.agent}"`);
    if (!cmd.id.startsWith(`${cmd.agent}.`)) problems.push(`${cmd.id}: id deveria começar com "${cmd.agent}."`);
    if (cmd.utterances.length < 8) problems.push(`${cmd.id}: só ${cmd.utterances.length} utterances (mínimo 8)`);
    if (cmd.keywords.length === 0) problems.push(`${cmd.id}: sem keywords`);
    if (!cmd.description.trim()) problems.push(`${cmd.id}: sem descrição`);

    const dupUtt = cmd.utterances.filter((u, i) => cmd.utterances.indexOf(u) !== i);
    if (dupUtt.length) problems.push(`${cmd.id}: utterances duplicadas: ${dupUtt.join(' | ')}`);
  ...
  for (const cmd of c.commands) {
    for (const other of cmd.confusableWith ?? []) {
      if (!seen.has(other)) problems.push(`${cmd.id}: confusableWith aponta para id inexistente "${other}"`);
    }
  }
