...
export function explainRoute(result) {
    ...
    const rows = result.candidates.map((candidate, i) => {
        ...
        if (hasRerank)
            row.push(b.rerank !== undefined ? fmt4(b.rerank) : '-');
        row.push(formatRanks(candidate), candidate.command.agent, commandIdToToolName(candidate.commandId), b.matchedKind, `"${truncate(b.matchedText, 56)}"`);
        return row;
    });
