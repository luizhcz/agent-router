/**
 * Diversificação do resultado — DESLIGADA por padrão (Brief §7).
 *
 * Sob gold único, recall@K é maximizado ordenando estritamente por relevância;
 * qualquer diversificação só pode *remover* candidatos relevantes do top-K. Por
 * isso `maxPerAgent` default é 0 e o MMR fica exportado mas nunca é chamado pelo
 * pipeline padrão — existe para o caso, medido em conjunto rotulado, de clusters
 * de near-duplicatas do mesmo agente saturarem o top-5.
 */
import type { Catalog, CommandIndex } from '../types.js';
 ...
 */
export function capPerAgent<T extends { commandId: string }>(
  candidates: T[],
  maxPerAgent: number,
  catalog: Catalog,
): T[] {
  if (maxPerAgent <= 0) return candidates;

  const agentOf = new Map<string, string>(catalog.commands.map((c) => [c.id, c.agent]));
 ...
 */
export function mmr<T extends { commandId: string; score: number }>(
  candidates: T[],
  lambda: number,
  topK: number,
  index: CommandIndex,
...
function buildCentroids(index: CommandIndex): Map<string, Float32Array> {
  ...
  for (const v of index.vectors) {
    ...
    for (let d = 0; d < dim; d++) acc[d]! += v.vector[d]!;
  }
  ...
  for (const acc of sums.values()) {
    ...
    n = Math.sqrt(n) || 1;
    for (let d = 0; d < dim; d++) acc[d]! = acc[d]! / n;
  }
  return sums;
}

function cosine(centroids: Map<string, Float32Array>, a: string, b: string): number {
