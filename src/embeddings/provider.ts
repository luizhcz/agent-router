...
import { createLocalProvider, type LocalEmbeddingOptions } from './local.js';
import { createOpenAIProvider, type OpenAIEmbeddingOptions } from './openai.js';
import { createCohereProvider, type CohereEmbeddingOptions } from './cohere.js';

/**
 * União discriminada por `kind`. A fábrica `createEmbeddingProvider` despacha
 * para a implementação certa. Cada variante mora no seu próprio arquivo e
 * reexporta o tipo das opções aqui para compor a união.
 */
...
export function dotProduct(
  ...
  const n =
    len ?? Math.min(a.length - offsetA, b.length - offsetB);
  let sum = 0;
  for (let i = 0; i < n; i++) {
...
// ---------------------------------------------------------------------------
// HTTP com retry (compartilhado por openai.ts e cohere.ts)
// ---------------------------------------------------------------------------

export interface RetryOptions {
  /** Rótulo do provider para mensagens de erro claras. */
  providerLabel: string;
  /** Máximo de tentativas totais (1 = sem retry). */
  maxRetries: number;
...
export async function fetchJsonWithRetry(
  ...
  for (let attempt = 0; attempt < opts.maxRetries; attempt++) {
    ...
    lastErr = new Error(
      `${opts.providerLabel}: HTTP ${res.status} ${res.statusText}${body ? ` — ${body}` : ''}`,
    );
...
export function chunk<T>(items: T[], size: number): T[][] {
  if (size <= 0) throw new Error(`chunk: tamanho de bloco inválido: ${size}`);
  const out: T[][] = [];
