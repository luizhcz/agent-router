...
function isHardToken(raw: string): boolean {
  ...
  if (raw === upper && raw !== raw.toLowerCase() && raw.length >= 2) return true;
...
export interface TokenizeOptions {
  /** Remover stopwords (siglas/tickers nunca são removidos). Default `true`. */
  removeStopwords?: boolean;
  /** Aplicar `lightStem` (só a tokens não-siglas). Default `false` (ver brief). */
  stem?: boolean;
}
 ...
 */
export function tokenize(s: string, options: TokenizeOptions = {}): string[] {
  ...
  for (const raw of raws) {
    ...
    if (!hard && removeStopwords && STOPWORDS.has(lower)) continue;
