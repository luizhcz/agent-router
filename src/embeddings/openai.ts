...
/**
 * Provider HTTP da OpenAI Embeddings API (fetch nativo). Modelos v3 são
 * simétricos — sem prefixo query/passage — então `asymmetric = false` e as duas
 * rotas fazem a mesma chamada. Suportam o parâmetro `dimensions` (truncamento
 * Matryoshka); truncar desfaz a normalização, por isso re-normalizamos sempre.
 */
export interface OpenAIEmbeddingOptions {
  kind: 'openai';
  /** Padrão `text-embedding-3-small` (1536 dims full). */
  model?: string;
  /** Dims de saída. Se omitido, usa o full do modelo (3-small=1536, 3-large=3072). */
  dimensions?: number;
  /** Chave. Padrão: `process.env.OPENAI_API_KEY`. */
  apiKey?: string;
  /** Base da API. Padrão `https://api.openai.com/v1`. */
  baseUrl?: string;
  /** Itens por requisição. Limite da API é 2048; padrão 2048. */
  maxBatchSize?: number;
 ...
 */
function resolveDimensions(model: string, explicit: number | undefined): number {
  if (explicit !== undefined) return explicit;
  const full = MODEL_FULL_DIMENSIONS[model];
  if (full !== undefined) return full;
  throw new Error(
    `Modelo de embedding OpenAI '${model}' não tem largura full conhecida. Passe ` +
...
export async function createOpenAIProvider(
  ...
  if (!apiKey) {
    throw new Error(
      'Provider de embedding OpenAI precisa de uma chave. Defina a variável de ' +
  ...
  async function embed(texts: string[]): Promise<Vector[]> {
